...
weight = {'first':2.5, 'second':2.5, 'third':2}
...
fourth_weight = {"testA.txt":2, "testB.txt":4, "testC.txt":2, "testD.txt":2, "testE.txt":2, "testF.txt":2, "testG.txt":2, "testH.txt":2, "testI.txt":2}
...
def fourth_grade(dirname):
    ...
    if not os.path.isfile("fourth"):
        print "Executable %s missing. Please check the compilation output."%("fourth")
	os.chdir(prevdir)
	print "Score is ", score
	print ""
	print ""
    ...
    test_dir = test_cases_directory + 'fourth/' 
    ...
    for test in list_tests:        
        ...
        ret = "";
        try:
            command_string = "./fourth " + train_file + " " + test_dir + test
	...
	#time.sleep(10)
        ...
        if compare_string_file(result_file, ret, show_difference=True):
            score = score + fourth_weight[test];
